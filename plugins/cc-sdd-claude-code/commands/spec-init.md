...
3. **Initialize Files Using Templates**:
    - Read template files from settings directory:
      - Try `{{KIRO_DIR}}/settings/templates/specs/init.json` first (traditional installation)
      - Fall back to `../.claude-plugin/settings/templates/specs/init.json` (marketplace installation)
      - Same fallback logic for `requirements-init.md`
